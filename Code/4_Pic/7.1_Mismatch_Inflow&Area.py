...
if __name__ == '__main__':
    ...
    # 提取数据
    inflow_deficit = summary['deficit_inflow_mean']/100
    inflow_surplus = summary['surplus_inflow_mean']/100
    ...
    # 绘制左侧气泡图
    x_labels = ['Deficit', 'Surplus']
    bubble_data = np.array([inflow_deficit[::-1], inflow_surplus[::-1]]).T  # 转置矩阵

    x_positions = np.array([0, 1])
    ax1.set_xticks(x_positions)

    # 绘制左侧子图（气泡热图），缩小气泡
    scatter = ax1.scatter(np.tile(x_positions, len(bubble_data)), np.repeat(np.arange(len(bubble_data)), 2),
                          s=bubble_data.flatten() * 70, c=bubble_data.flatten(), cmap='Greens', alpha=0.8, edgecolor='black')
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(path + '/Result/pic/sigma=' + str(sigma) + '/Inflow/NumPerShelter/mismatch_inflow_area.pdf')
    plt.savefig(path + '/Result/pic/sigma=' + str(sigma) + '/Inflow/NumPerShelter/mismatch_inflow_area.png', dpi=300)
