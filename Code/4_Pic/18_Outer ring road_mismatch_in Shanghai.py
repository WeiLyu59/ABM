# -*-coding:utf-8 -*-
# python 3.9
# 上海mismatch区域叠加outer ring

"""
# @File       : 18_Outer ring road_mismatch_in Shanghai.py
# @software   : PyCharm  
# @Time       ：2025/4/17 19:52
# @Author     ：Wei Lyu
"""
import pandas as pd
...
if __name__ == '__main__':
    ...
    popPredShelter = pd.read_csv(data_path + '/inflowPredShelter_MatchLabel.csv', header=0)
    popPredShelter['Tid'] = popPredShelter['Tid'] - 1  # 推演的时候Tid比shp里的Tid大1
    merged = shp.merge(popPredShelter, on='Tid')  # 合并 inflow DataFrame 和 shapefile
    boundary_shp = gpd.read_file(path + '/cities_21/' + city + '/' + city + '.shp')
    county_shp = gpd.read_file(path + '/cities_21/' + city + '/' + city + '_county.shp')
    outer_ring_shp = gpd.read_file(path + '/cities_21/' + city + '/Outer_ring_road.shp')

    # 设置坐标系为 WGS84 (经纬度)
    merged = merged.to_crs(epsg=3857)
    boundary_shp = boundary_shp.to_crs(epsg=3857)
    county_shp = county_shp.to_crs(epsg=3857)
    outer_ring_shp = outer_ring_shp.to_crs(epsg=3857)
    ...
    county_shp.boundary.plot(ax=ax, edgecolor='k', linewidth=0.7)  # 绘制行政区
    boundary_shp.boundary.plot(ax=ax, edgecolor='k', linewidth=0.7)  # 绘制边界
    outer_ring_shp.boundary.plot(ax=ax, edgecolor='#1FF19F', linewidth=0.7)  # 绘制外环路
    ...
    plt.savefig(path + '/Result/pic/For manu/Shanghai_Outer ring_overlay mismatches.pdf')
    plt.show()
    # break
    plt.close()